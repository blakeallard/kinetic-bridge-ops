...
| Deluge parser/QC parity | No Zoho-native equivalent | Port strict parsing, embedded Zoho AI parsing, reasoned QC skips, and selected-only payload input | Add | Local Stage 8 projection matches Python; Zoho sandbox execution still required before workflow wiring |
| Deluge runtime validation | Not applicable | Execute only in an authorized inert custom-function tester and compare the Stage 8 projection | Pending / blocked | `zoho_all` exposes no Deluge/Flow/function runner; do not claim runtime parity until returned output is recorded |
